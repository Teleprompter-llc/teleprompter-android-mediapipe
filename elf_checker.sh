...
# usage: check_elf_alignment.sh [path to *.so files|path to *.apk|path to *.aar]
...
usage() {
  ...
  echo
  echo "Usage: ${progname} [input-path|input-APK|input-APEX|input-AAR]"
}
...
# --- Start of APK/APEX/AAR processing for temp directory ---
# This section extracts the APK/APEX/AAR content into a temporary directory
...
if [[ "${dir}" == *.aar ]]; then
  ...
  echo
  echo "Recursively analyzing AAR: $dir"
  echo

  dir_filename=$(basename "${dir}")
  tmp=$(mktemp -d -t "${dir_filename%.aar}_out_XXXXX")
  
  # AAR files contain native libraries in 'jni/' directory
  # Extract the AAR contents
  unzip -q "${dir}" -d "${tmp}" 2>/dev/null
  
  # Check if jni directory exists
  if [ -d "${tmp}/jni" ]; then
    dir="${tmp}/jni"
    echo "Found native libraries in jni/ directory"
  else
    # Some AARs might not have native libraries
    echo "Warning: No jni/ directory found in AAR. This AAR may not contain native libraries."
    dir="${tmp}"
  fi
fi
# --- End of APK/APEX/AAR processing for temp directory ---
...
for abi in "${ABIS[@]}"; do
  ...
  if [ -d "${abi_path}" ]; then
    ...
    abi_matches="$(find "${abi_path}" -type f -name "*.so" 2>/dev/null)"
